...
const VLAB = (() => {
  ...
  --------------------------------------------------------------------- */
  const auth = {
    // Real Supabase auth (see assets/js/supabase-client.js). `username` gets
    // mapped to `${username}@vitalslab.internal` so staff never have to
    // remember a real email address.
    async login(username, password, expectedRole){
      const email = `${username.trim().toLowerCase()}@vitalslab.internal`;
      return await window.SB.auth.login(email, password, expectedRole);
    },
    async logout(){
      await window.SB.auth.logout();
    },
    async current(){
      return await window.SB.auth.current();
    },
    async requireAuth(){
